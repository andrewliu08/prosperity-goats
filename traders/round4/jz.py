...
class CoconutTrader:
    ...
    def run(self, state: TradingState) -> None:
        prices = {
            product: manager.get_mid_price() for product, manager in self.managers.items()
        }
        positions = {
            product: manager.get_position()
            for product, manager in self.managers.items()
        ...
        RISK_FREE = 0.001  # Risk-free rate
        T = 246/252 # Time to maturity
        volatility = self.find_volatility(10000, 10000, T, RISK_FREE, target_price=637.7)
        pred_price = self.black_scholes(S, K, T, RISK_FREE, volatility)
