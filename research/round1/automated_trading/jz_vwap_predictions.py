...
all_trades = pd.concat([trades_2, trades_1])
...
features["ema_weighted_price"] = (
    features["weighted_trade_price"].ewm(span=10, adjust=False).mean()
)
...
features["log_returns"] = np.log(
    features["weighted_trade_price"] / features["weighted_trade_price"].shift(1)
)
# Handling any infinite or NaN values that might arise from division by zero or log of zero
features["log_returns"] = (
    features["log_returns"].replace([np.inf, -np.inf], np.nan).fillna(0)
)
